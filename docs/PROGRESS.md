...
> **Dừng tại:** 2 slice committed trên prod `484eb0c3`, **clean build PASS** (api tsc 0 lỗi + web webpack 0 lỗi), reviewed. **CHỜ DEPLOY** — chưa chạy safe-deploy, prod runtime vẫn `484eb0c3`. **Code-only, KHÔNG migration mới** (0431 đã apply, Drizzle chỉ khai cột DB đã có).
> **Bước tiếp CHÍNH XÁC:** (1) Deploy gom 2 slice: `cd claude/pos-system && bash scripts/safe-deploy.sh n8n/005-wecha-pos-zalo-keypos` (KHÔNG cần apply SQL). (2) Sau deploy: tag + verify health 200 + click-test 1 nút gate (NV thường không thấy nút duyệt). (3) **Slice 3 dang dở:** CSDL/branch còn thiếu (promotions/voucher/gift-card thêm `branch_id` read-filter) + nút **nạp thẻ** gift-card (POST /gift-cards/:id/recharge) + export CSV campaigns/gift-cards + vouchers/codes join customerName + dashboard recentOrders/export-excel. (4) Bảo mật: ĐỔI pass DB (còn trong history) + xác nhận repo private.
...
> **Hệ khác cần biết:** prod vẫn `484eb0c3` tới khi deploy 2 slice. Repo `erp-orchestration` PROGRESS sync tay (scrub IP/pass trước push).
