...
### 🏁 BÀN GIAO MỚI NHẤT ⏰ 2026-07-01 06:55 +0700 (phiên DÀI tự chủ — vá 5/5 lỗ hổng + HR + FE + Đợt 6 + web-wecha infra) ⭐ ĐỌC ĐẦU TIÊN
> **Vừa làm:** 6 đợt permission-hardening (verify-first→lockout-check PROD THẬT→reviewer đối kháng→fix sót→commit). Đợt 1-2 LIVE Batch 1. Đợt 3 P0+P1 HR gate XONG (lương/PII + chấm công/nghỉ/tăng ca + IDOR fix). Đợt 4 LMS courses. Đợt 5 sàn TMĐT (OMIT api_key/secret). FE ẩn-nút HR 19 trang/LMS/sàn. Đợt 6 manufacturing (wire 8/12 STUB + branch + CSDL fail-open). Hạ tầng **web-wecha DB @5434 SET XONG**. C19 duyệt (VIEC-C19.md).
> **Dừng tại:** Pending batch **6 commit `548c3c4c..5edf42cd`** trên prod `ad7a072f` (CHƯA deploy). Mig **0436-0439**. Tree SẠCH. api+web tsc PASS. ⚠️ **Đợt 6 (`5edf42cd`) CHƯA qua reviewer** — wire STUB cần soát trước deploy. Code push GitHub `erp-wecha-pos` HEAD=5edf42cd.
> **Bước tiếp CHÍNH XÁC:** (1) **DEPLOY Batch 2** (xem "DEPLOY BATCH 2" mục dưới — apply 0436→0439 → `safe-deploy.sh n8n/005-wecha-pos-zalo-keypos` → NV login lại). 🔴 NÊN reviewer Đợt 6 `5edf42cd` (wire STUB manufacturing) TRƯỚC khi deploy. (2) Đợt 7 KH/SP · Đợt 8 Marketing · Đợt 9 Lõi — phần lớn là CSDL `database_id` = **G4 CHỜ LETRI DUYỆT** (đổi kết quả query). (3) HR audit-log (3 điểm: payslip-pdf/employee-PII/bhxh-export). (4) **API ERP↔web** (read-only đọc SP/giá + nhận CRM) cho C19/nguyenlieuantoan/phache. (5) 4 report manufacturing cần spec (equipment table/yield/cost/plan-vs-actual) — Letri cấp công thức.
> **Cạm bẫy:** 🔴 **G4 chưa duyệt** → Đợt 7-9 CSDL chưa làm (chờ Letri). seed quyền TRƯỚC guard + lockout verify PROD từng đợt (role 'Kế toán trường' typo ĐÚNG prod). Reviewer bắt sót THẬT mỗi đợt (cost-leak lots/excel · secret 3 disconnect · HR contracts/advances) → **LUÔN reviewer trước deploy**. Đợt 6 CHƯA reviewer. safe-deploy rsync working-tree → **chốt tree sạch trước deploy**.
> **Hệ khác:** prod `ad7a072f` tới khi deploy Batch 2. **web-wecha DB @5434** (container Docker cô lập, C19 build được — credential `/opt/web-wecha/db.env`, CẤM pos_db). C19 đã duyệt. Disk VPS 80% (để ý). erp-orchestration sync PROGRESS.

### 🏁 BÀN GIAO ⏰ 2026-06-30 tối (KẾ HOẠCH LỚN: CSDL+branch+quyền TOÀN HỆ + quyền-riêng-từng-ổ + web API + Antigravity) ⭐ ĐỌC KỸ — KHÔNG BỎ SÓT
...
> **Dừng tại:** ✅ **LIVE 30/6 15:16** — prod runtime deploy=`4f2c20da` (tag `sales-qa-deadbtn-live-20260630-1516`). **Code prod thực = `831c1bee`** (= 4f2c20da + branch_id read-filter promotions/gift-card/voucher + customerName subquery + xuất CSV voucher — các thay đổi này LIVE qua rsync working-tree, sau đó commit `831c1bee` sync git↔prod, push erp-wecha-pos HEAD=831c1bee). Deploy SUCCESS (api/web 200, pm2 15:15). Rollback `.old-20260630-150735` + tar. KHÔNG migration. Working tree SẠCH (chỉ còn untracked docs/ — không phải code). 🔒 .env tracked = .env.example (template) + .env.production (public) — KHÔNG secret thật.
...
[1/7 ⏰ 2026-07-01 06:24 +0700] [OPUS-CEO-ERP] **ĐỐI CHIẾU THẬT bàn giao C19 (workflow 4 agent)** — KHÔNG tin báo cáo suông, đã grep-verify. ✅ **3 nơi lưu CÓ THẬT, đúng claim chính:** handover `HANDOVER-C19-20260630.md` đủ 8 mục a-h + 2 nút verify tay (dòng 7-68); PROGRESS entry dòng 842 trỏ đúng handover; memory `project_c19_wecha_fullstack_pivot.md` có đủ 5444 + DB-mới-web-wecha + 58 trà. ⚠️ **Lệch NHỎ (hình thức):** handover ghi "9 luật" nhưng đếm 8 gạch đầu dòng; bước-tiếp chưa tới file:dòng tuyệt đối; entry PROGRESS viết 1 đoạn dày thay 5 dòng nhãn. 🔴 **2 CẢNH BÁO PHIÊN SAU PHẢI XỬ TRƯỚC KHI COMMIT:** (1) **working tree 1019 file uncommitted LẪN nhiều hệ khác** (docs/, orchestrator-ceo/, claude design POS-V4 — commit cuối acc9d55 là C13) → khi commit web PHẢI `git add` CỤ THỂ file `antigravity-project/` (luật git-add-specific + cấm reset-hard code C khác), TUYỆT ĐỐI không `git add -A`. (2) **con số "58 trà" + "DB sạch" KHÔNG verify được từ source** — đếm 2 file data (wholesale-teas.ts 30 + retail-tea-content.ts 26) ra ~56, "58" chỉ là trạng thái DB local 5444 → phiên sau khi sinh migration+seed PHẢI query DB đối chiếu số thật. (3) "typecheck PASS" chỉ xác nhận script `tsc --noEmit` tồn tại (package.json:10), chưa chạy lại. Phân biệt cổng: DB LOCAL Mac=5444 · **DB ERP prod=5433 (CẤM đụng)**. ⚠️ **CẬP NHẬT 1/7 (CEO ERP set hạ tầng):** web DB đã set xong — container Docker `web_wecha` **@5434** (bind 127.0.0.1, trống 0 bảng), credential `/opt/web-wecha/db.env`; tên prod `web_wecha` ≠ local `wecha_web` (đảo chữ, đừng nhầm). Ghi chú cũ "web-wecha=5433" của em SAI → đã sửa handover+memory. Kết luận: **bàn giao ĐẠT, đủ để phiên sau tiếp việc** — chỉ siết lúc commit + verify lại con số catalog.

[30/6 ⏰ 2026-06-30 20:46 +0700] [C19] **BÀN GIAO PHIÊN (token 96%)** — chi tiết: `orchestrator-ceo/ceos/c19-wecha-web/HANDOVER-C19-20260630.md` (đọc KỸ, có: hệ/code/DB/tích hợp/sai-sót/luật). **Tiếp các fix audit + i18n /en** (typecheck PASS, CHƯA commit/deploy): i18n /en hoàn tất cụm thanh toán + landing `/tra/[slug]` 17 chuỗi + nhãn giá "Contact for price" (5 nơi) + trà sữa Dine-in + **drink option/group `nameEn` + drink `descriptionEn`** (commerce-queries + AddToCartButton + menu/[slug]). **CATALOG TRÀ (catalogVAT.pdf):** đọc lại NÉT CAO (lần trước low-res SAI tên: "Bưởi Đào"→"Xuân Xanh", "Bạch Đơn Tuyết"→"Xanh Shan Tuyết", bịa "Hoa Anh Đào", sót "Trà Xanh Không Nhài") → **xoá 35 sai + thêm lại 45 ĐÚNG** (catalog có 50 món/6 nhóm, 5 trùng đã có) → **tổng 58 retail_tea**, giá "Liên hệ". **AUDIT MOBILE 390px:** home/nha-may/mua-tra/landing = 0 chữ vô hình + không tràn ngang. 🔴 **PHÁT HIỆN+FIX lỗi "load nhầm DB":** local có 2 Postgres (@14 pos_db chiếm 5432, @16 wecha_web crash) → app báo `role wecha không tồn tại`. **Đã khôi phục @16 ở cổng 5444 + .env.local trỏ 5444** (data nguyên vẹn). 🔴 **LUẬT MỚI Letri (30/6):** KHÔNG tự sửa/xoá/tạo trên VPS — xin Letri/hỏi CEO ERP; **deploy = tạo DB MỚI `web-wecha`** (không đụng pos_db/n8n). **CÒN:** ~108 medium/low + sinh migration+seed (gồm 58 trà) + build → deploy (chờ phép). 2 nút (block editor/bulk upload) cần bấm tay verify.
