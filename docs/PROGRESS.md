...
> **Dừng tại:** prod=HEAD=`aff3b5d1`. **Working tree CHƯA commit (WIP, CHƯA review/build):** ĐỢT3 read-filter (10 BE+6 FE) + **5 worker sales ĐANG CHẠY** fix 65 gap + bulk (orders/quotes/promotions/vouchers/gift-cards/returns/reservations/attributes/barcodes/reports). products/customers đã REVERT (trùng — đã có sẵn CSDL/branch+bulk).
