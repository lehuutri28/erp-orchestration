...
### 🏁 BÀN GIAO MỚI NHẤT ⏰ 2026-06-29 ~14:xx (phiên DÀI — CSDL scoping + audit bán hàng + repo điều phối)
...
> **Dừng tại:** prod runtime=`aff3b5d1` (chưa deploy WIP). **WIP đã COMMIT `173a57b7` (pushed, branch n8n/005) — CHƯA build/review/deploy:** ĐỢT3 read-filter + **5/5 worker sales XONG** fix 65 gap + bulk (đơn-hàng/B2B [fix giảm-giá-% sai tiền], báo giá [giảm-giá-%], km/voucher/thẻ [soft-delete+bulk+schema cột 0431], đặt-bàn/trả-hàng, thuộc-tính/mã-vạch/báo-cáo). products/customers đã REVERT (trùng — ĐÃ có sẵn CSDL/branch+bulk). Mig đã commit: 0429+0431 (CHƯA apply).
...
**Cạm bẫy:** migration từ **0408**. safe-deploy KHÔNG tự chạy mig → apply tay (scp+psql). req.user**.sub**. db.execute()=QueryResult→.rows. Route `:id` đặt SAU route literal. Branch `n8n/005-wecha-pos-zalo-keypos`, code `claude/pos-system`, VPS `<VPS_USER>@<VPS_IP>` `pos_db`.
...
1. **Apply migration 0406 TRƯỚC** (an toàn, idempotent):
   ```
   cd /Users/letri/Projects/erp-project && scp claude/pos-system/apps/api/src/database/migrations/0406_haccp_deviations_measurement_fields.sql <VPS_USER>@<VPS_IP>:/opt/pos-system/apps/api/src/database/migrations/ && ssh -t <VPS_USER>@<VPS_IP> 'cd /opt/pos-system/apps/api && set -a && . ./.env && set +a && psql "$DATABASE_URL" -f src/database/migrations/0406_haccp_deviations_measurement_fields.sql'
   ```
...
**Cạm bẫy:** migration mới từ **0405**. orders giờ ĐÃ có deleted_at (soft-delete) — module khác có thể filter. quotes/quote_items vừa tạo (cấu trúc khớp commerce-quotes.ts). VPS pos_db <VPS_USER>@<VPS_IP>, deploy 8 bước (apply mig TRƯỚC code), safe-deploy đã thêm `--exclude=docs/`.
...
- VPS `<VPS_USER>@<VPS_IP>` DB `pos_db`. Deploy 5 bước.
